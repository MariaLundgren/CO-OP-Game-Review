...
## Open bugs
- On some mobile devices the select firld for choosing a rating on the review dosn't work properlly. The input field don't always put in the number that the user chooses but puts in another number instead. However, it is created a litte dropdown right underneath the input field where the user can put in the rating instead.  